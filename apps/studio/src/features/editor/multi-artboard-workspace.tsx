...
export const MultiArtboardWorkspace = memo(function MultiArtboardWorkspace({
  ...
  const [renderRegistry] = useState(() => new MultiArtboardRenderRegistry())
  useEffect(
    () => () => {
      void renderRegistry.dispose()
    },
    [renderRegistry]
  )
  const pagesById = new Map(document.pages.map((page) => [page.id, page]))
  return (
    <MultiArtboardRenderRegistryContext.Provider value={renderRegistry}>
      <div
        className="relative"
