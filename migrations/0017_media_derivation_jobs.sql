...
CREATE UNIQUE INDEX idx_media_assets_derivation_source_identity
ON media_assets(workspace_id, id, content_hash);

CREATE UNIQUE INDEX idx_media_assets_derivation_output_identity
ON media_assets(workspace_id, id, content_hash, media_type, width, height);
...
CREATE TABLE media_derivation_requests (
  workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  idempotency_key TEXT NOT NULL
    CONSTRAINT chk_media_derivation_requests_key
    CHECK (
      length(idempotency_key) BETWEEN 1 AND 128
      AND idempotency_key NOT GLOB '*[^A-Za-z0-9._:-]*'
    ),
  request_fingerprint TEXT NOT NULL
    CONSTRAINT chk_media_derivation_requests_fingerprint
    CHECK (
      length(request_fingerprint) = 64
      AND request_fingerprint NOT GLOB '*[^a-f0-9]*'
    ),
  ...
  FOREIGN KEY (workspace_id, job_id, request_fingerprint)
    REFERENCES media_derivation_jobs(
      workspace_id, id, request_fingerprint
    ) ON DELETE CASCADE
);

CREATE TABLE media_derivation_attempts (
  id TEXT PRIMARY KEY
    CONSTRAINT chk_media_derivation_attempts_id
    CHECK (
      length(id) BETWEEN 36 AND 112
      AND substr(id, 1, 19) = 'derivation-attempt-'
      AND substr(id, 20) NOT GLOB '*[^A-Za-z0-9_-]*'
    ),
  workspace_id TEXT NOT NULL,
  job_id TEXT NOT NULL,
  attempt_number INTEGER NOT NULL
    CONSTRAINT chk_media_derivation_attempts_number
    CHECK (typeof(attempt_number) = 'integer' AND attempt_number >= 1),
  provider_execution_id TEXT
    CONSTRAINT chk_media_derivation_attempts_execution
    CHECK (
      provider_execution_id IS NULL
      OR length(trim(provider_execution_id)) BETWEEN 1 AND 512
    ),
  state TEXT NOT NULL
    CONSTRAINT chk_media_derivation_attempts_state
    CHECK (state IN ('running', 'succeeded', 'failed', 'cancelled')),
  safe_failure_code TEXT
    CONSTRAINT chk_media_derivation_attempts_failure_code
    CHECK (
      safe_failure_code IS NULL
      OR (
        length(safe_failure_code) BETWEEN 1 AND 128
  ...
  retryable INTEGER NOT NULL DEFAULT 0
    CONSTRAINT chk_media_derivation_attempts_retryable
    CHECK (retryable IN (0, 1)),
  started_at TEXT NOT NULL,
  finished_at TEXT,
  CONSTRAINT chk_media_derivation_attempts_settlement
    CHECK (
      (state = 'running' AND finished_at IS NULL
        AND safe_failure_code IS NULL AND retryable = 0)
  ...
  CONSTRAINT chk_media_derivation_attempts_timestamps
...
CREATE TABLE media_derivation_provenance (
  ...
  PRIMARY KEY (workspace_id, output_asset_id),
  UNIQUE (workspace_id, derivation_job_id),
  FOREIGN KEY (workspace_id, source_asset_id, source_content_hash)
    REFERENCES media_assets(workspace_id, id, content_hash) ON DELETE RESTRICT,
  FOREIGN KEY (
...
CREATE TRIGGER media_derivation_attempts_legal_state_transition
BEFORE UPDATE OF state ON media_derivation_attempts
WHEN OLD.state <> NEW.state AND NOT (
  OLD.state = 'running'
  AND NEW.state IN ('succeeded', 'failed', 'cancelled')
)
BEGIN
  SELECT RAISE(ABORT, 'media_derivation_attempt_transition_invalid');
END;

CREATE TRIGGER media_derivation_attempts_identity_immutable
BEFORE UPDATE OF workspace_id, job_id, attempt_number, started_at
ON media_derivation_attempts
BEGIN
  SELECT RAISE(ABORT, 'media_derivation_attempt_identity_immutable');
END;

CREATE TRIGGER media_derivation_attempts_settlement_immutable
BEFORE UPDATE ON media_derivation_attempts
WHEN OLD.state <> 'running'
BEGIN
  SELECT RAISE(ABORT, 'media_derivation_attempt_settlement_immutable');
END;

CREATE TRIGGER media_derivation_requests_immutable
BEFORE UPDATE ON media_derivation_requests
BEGIN
  SELECT RAISE(ABORT, 'media_derivation_request_immutable');
END;
...
CREATE INDEX idx_media_derivation_requests_job
ON media_derivation_requests(workspace_id, job_id, created_at);

CREATE INDEX idx_media_derivation_attempts_job
ON media_derivation_attempts(workspace_id, job_id, attempt_number DESC);
