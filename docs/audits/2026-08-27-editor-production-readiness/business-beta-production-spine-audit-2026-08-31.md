...
the API. BB-00 records the current ordered filename and SHA-256 digest of all 17
migrations. The shared validator rejects a missing, extra, reordered,
renumbered, or mismatched current SQL file before remote inspection. A
coordinated change to a SQL file and its manifest digest still passes. The
deployment snapshot detects lineage changes only during one plan/apply run, so
a trusted historical anchor remains open.
...
only at the time of that capture. Wrangler authentication failed during this
reconciliation, and no successful current remote inventory or D1 inspection
was retained. Current remote state must be inspected before anyone states the
exact pending suffix.
...
| Deployed D1 migrations                            | Deployment tooling validates configured resource identities, current migration ordinals and SQL/manifest agreement, a remote ledger prefix, a stable plan/apply suffix, package builds, and a zero-pending post-deploy state. Individual local migration harnesses cover the important upgrades through `0017`. The plan/apply snapshot detects lineage changes during that run.                                                                             | The SQL files and manifest are mutable together, so they do not provide a trusted historical anchor. The last retained remote baseline records an exact prefix through `0011` at capture time. The current pending suffix is unknown until authenticated read-only inspection succeeds. Applying the inspected suffix and deploying matching Workers remain production-write actions. |
...
1. `BB-00 migration lineage`. Record every current migration filename and
   SHA-256 digest in a checked-in manifest. Static deployment preflight rejects
   missing, extra, reordered, or mismatched current files. The remaining exit is
   a trusted historical anchor that cannot be changed with the SQL and manifest.
...
- Cloudflare D1 records applied migration filenames and applies unapplied SQL in
  order. The project records content hashes for current-tree self-consistency.
  The mutable SQL/manifest pair does not prove historical immutability.
...
BB-00 records the checked-in D1 lineage in `migrations/manifest.json`. Each entry
contains the current ordered filename and SHA-256 digest. The shared validator
rejects missing, extra, reordered, renumbered, or current SQL files that disagree
with the current manifest before Cloudflare inspection or deployment can
proceed. Static preflight loads the same validator. The production deployment
runner snapshots the lineage digest during planning and rechecks it immediately
before applying a remote suffix.

This is a current-tree and same-run guarantee. A commit that changes historical
SQL and updates its manifest digest can pass. BB-00 does not yet provide a signed,
external, protected-branch, or other trusted historical anchor.
...
read-only production plan, but Wrangler authentication failed and no successful
current remote inventory or D1 inspection was retained. BB-01 through BB-06
remain separately authorized evidence gates.
...
1. **Lineage anchor and authenticated read-only inspection.** Select the clean
   integration commit and establish or name a trusted historical anchor outside
   the mutable SQL/manifest pair. Authenticate Wrangler to the configured
   production account, run the static and remote-ready preflights, and read
   `sqlite_schema` plus `d1_migrations`. Record the exact remote prefix and
   computed pending suffix. Stop on a missing anchor or ledger, divergent
   filename, digest mismatch, unexpected resource, or account mismatch. This
   item does not authorize migration application, deployment, Access changes,
   product writes, or Browser Rendering.
2. **Schema and Worker promotion.** Name the reviewed commit and the exact
   ...
   migrations, and a new retained read-only baseline. A stale plan, changed
