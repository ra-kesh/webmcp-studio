...
Status: **repository closure, persistence ownership/layout cutover, Recent/Trash controller/provider/view-model, and exact-ID opening independently approved; visible library and routed conflict/recovery UI remain open**
...
- The next implementation is metadata-only: independent active/deleted slots, 24-row opaque-cursor pages, repository-backed name search, one queued refresh rerun, stale completion rejection, sticky recovery inventory, soft-delete/Restore only, operation-local failures, explicit Load more, truthful no-preview tiles, and no purge UI. It reuses the approved hook-level transition owner for exact-ID open and must remove the transitional one-card list owner rather than run both.
