...
|     3 | CONFORM-01 / EXPORT-01    | Active. A real browser fixture gates fonts, images, Fabric sync and paint. SVG sizing, canonical-line ownership, live text reflow, auto-width wrapping and Fabric/CSS line-height semantics are repaired and independently accepted. Studio now reaches the local auxiliary Renderer through a real service binding; sequential smoke requests return valid PNG and two-page PDF bytes through remote Browser Rendering. The retained JPEG-derived pixel report predates the final text repair. | Retain lossless Renderer PNG/PDF captures, verify JSON round-trip and page order, separate text rasterization tolerance from geometry, compare React/Fabric/Renderer output, then repeat the same gate against the deployed Renderer. |
