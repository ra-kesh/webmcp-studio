...
|     3 | CONFORM-01 / EXPORT-01    | Active. A real browser fixture now gates fonts, images, Fabric sync and paint. SVG intrinsic sizing, canonical-line ownership, live text reflow, auto-width wrapping and Fabric/CSS line-height semantics are repaired and independently accepted. The retained JPEG-derived pixel report predates the final canonical-line repair. Local Studio export currently has no `RENDERER` binding. | Add lossless captures, exercise Renderer PNG/PDF through a real service binding, verify JSON round-trip and page order, separate text rasterization tolerance from geometry, then repeat against deployed Browser Rendering. |
